...
def _collect_attempted_queries(state: ResearchState) -> set[str]:
    attempted = {_normalize_query_text(item) for item in state.get("attempted_search_queries", []) if str(item).strip()}
    for plan_item in state.get("search_plan", []) + state.get("supplementary_queries", []):
        if isinstance(plan_item, dict):
            query = _normalize_query_text(str(plan_item.get("query", "")))
            if query:
                attempted.add(query)
    for trace in state.get("web_search_trace", []) + state.get("local_rag_trace", []):
        if isinstance(trace, dict):
            query = _normalize_query_text(str(trace.get("query", "")))
            if query:
                attempted.add(query)
    return attempted


def _filter_new_supplementary_queries(state: ResearchState, candidates: list[dict]) -> tuple[list[dict], list[str]]:
    attempted = _collect_attempted_queries(state)
    kept: list[dict] = []
    seen = set(attempted)
    for item in candidates:
        if not isinstance(item, dict):
...
def _quality_filter_records(records: list[dict], source_type: str, fallback_query: str) -> tuple[list[dict], dict]:
    kept: list[dict] = []
    stats = {
        "raw_count": len(records),
        "quality_kept_count": 0,
    ...
    for record in records:
        query = str(record.get("search_query") or fallback_query)
        if source_type == "web":
            filtered, item_stats = _filter_web_records(query, [record])
        else:
            filtered, item_stats = _filter_local_records(query, [record])

        relevance_scores.append(float(record.get("relevance_score", 0.0) or 0.0))
        if filtered:
            kept.extend(filtered)
            stats["quality_kept_count"] += 1
        else:
            stats["quality_dropped_count"] += max(
                item_stats.get("raw_count", 1) - item_stats.get("kept_count", 0),
                1,
            )

    if relevance_scores:
        stats["avg_relevance_score"] = round(sum(relevance_scores) / len(relevance_scores), 4)
    return kept, stats


def _merge_quality_stats(target: dict, quality_stats: dict) -> dict:
    target["quality_kept_count"] = target.get("quality_kept_count", 0) + quality_stats.get("quality_kept_count", 0)
    target["quality_dropped_count"] = target.get("quality_dropped_count", 0) + quality_stats.get("quality_dropped_count", 0)
    previous_raw = target.get("_quality_score_raw_count", 0)
    previous_avg = target.get("avg_relevance_score", 0.0)
    current_raw = quality_stats.get("raw_count", 0)
    current_avg = quality_stats.get("avg_relevance_score", 0.0)
    total_raw = previous_raw + current_raw
    if total_raw:
        target["avg_relevance_score"] = round(
            ((previous_avg * previous_raw) + (current_avg * current_raw)) / total_raw,
            4,
        )
        target["_quality_score_raw_count"] = total_raw
    return target


def _assess_retrieval_quality(state: ResearchState) -> tuple[str, list[str]]:
    web_stats = state.get("web_retrieval_stats", {})
    local_stats = state.get("local_retrieval_stats", {})
    evidence_count = len(state.get("evidence_pool", [])) or len(state.get("web_evidence", [])) + len(state.get("local_evidence", []))
    quality_kept = web_stats.get("quality_kept_count", 0) + local_stats.get("quality_kept_count", 0)
    raw_count = web_stats.get("raw_count", 0) + local_stats.get("raw_count", 0)
    avg_scores = [
        float(stats.get("avg_relevance_score", 0.0) or 0.0)
        for stats in (web_stats, local_stats)
        if stats.get("raw_count", 0) > 0
    ]
    avg_relevance = sum(avg_scores) / len(avg_scores) if avg_scores else 0.0

    reasons: list[str] = []
    if raw_count == 0:
        reasons.append("未检索到原始结果")
    if quality_kept == 0:
        reasons.append("相关性过滤后无可用结果")
    if evidence_count == 0:
        reasons.append("证据池为空")
    if raw_count > 0 and avg_relevance < 0.2:
        reasons.append("平均相关性评分过低")

    if reasons:
        return "low", reasons
    if evidence_count < 2:
        return "partial", ["可用证据数量偏少"]
    return "sufficient", []
...
def web_search_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    raw_count_before_quality = len(raw_records)
    raw_records, quality_stats = _quality_filter_records(raw_records, "web", state["query"])
    logger.info(
        "[web_search_node] 数据清洗后 | 基础过滤=%s | 相关性保留=%s",
        raw_count_before_quality,
        len(raw_records),
    )
    ...
    web_retrieval_stats["raw_count"] = web_retrieval_stats.get("raw_count", 0) + raw_count_before_quality
    web_retrieval_stats = _merge_quality_stats(web_retrieval_stats, quality_stats)
...
def local_rag_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    raw_count_before_quality = len(raw_records)
    raw_records, quality_stats = _quality_filter_records(raw_records, "local", state["query"])
    ...
    local_retrieval_stats["raw_count"] = local_retrieval_stats.get("raw_count", 0) + raw_count_before_quality
    local_retrieval_stats = _merge_quality_stats(local_retrieval_stats, quality_stats)
...
def analyze_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    retrieval_quality, quality_reasons = _assess_retrieval_quality(state)
    payload, content, messages = _invoke_json_agent(
        state,
        ...
        f"检索质量：{retrieval_quality}，原因：{json.dumps(quality_reasons, ensure_ascii=False)}\n"
    ...
    if retrieval_quality == "low":
        ...
        missing_gaps = list(missing_gaps) if isinstance(missing_gaps, list) else []
        for reason in quality_reasons:
            gap = f"检索质量不足：{reason}"
            if gap not in missing_gaps:
                missing_gaps.append(gap)
    analysis_summary = payload.get("analysis_summary", content)
    current_evidence_count = len(state.get("evidence_pool", []))
    previous_evidence_count = int(state.get("previous_evidence_count", 0) or 0)
    if state.get("iteration", 0) > 0 and current_evidence_count <= previous_evidence_count:
        no_new_evidence_rounds = int(state.get("no_new_evidence_rounds", 0) or 0) + 1
    else:
    ...
    return {
        ...
        "retrieval_quality_reasons": quality_reasons,
...
def reflect_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    raw_supplementary = payload.get("supplementary_queries", fallback["supplementary_queries"])
    if not isinstance(raw_supplementary, list):
        raw_supplementary = fallback["supplementary_queries"]
    supplementary_queries, attempted_queries = _filter_new_supplementary_queries(state, raw_supplementary)
    reflection_stop_reason = "" if supplementary_queries else "no_new_supplementary_queries"
    
    return {
        "iteration": state.get("iteration", 0) + 1,
        "supplementary_queries": supplementary_queries,
        "attempted_search_queries": attempted_queries,
        "reflection_stop_reason": reflection_stop_reason,
        "messages": messages,
    }
