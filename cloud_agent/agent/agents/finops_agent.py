...
"""
        ...
        facts = extract_finops_facts(result.get("messages", []))
        validated_content, validation_issues = validate_finops_response(final_message.content, facts)
        if validation_issues:
            final_message = final_message.model_copy(update={"content": validated_content})
            metadata = dict(metadata)
            metadata["finops_validation_issues"] = validation_issues
            print(
        ...
        return {"messages": [final_message], "next_agent": "", "metadata": metadata}
