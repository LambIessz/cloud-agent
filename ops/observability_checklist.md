...
| --- | --- | --- | --- | --- |
| SKIP | 启动 Prometheus + Grafana | `docker compose -f docker-compose.observability.yml up` | 容器启动，无配置加载错误 | Docker 运行态本轮跳过；compose config 已 PASS，真实运行改用 Ubuntu VM 原生 Prometheus/Grafana |
...
如果实际启动 Docker 观测栈受阻，优先完成以下低风险工作：

1. 优先使用 Ubuntu VM 原生 Prometheus + Grafana 路径，不继续卡 Docker Hub 拉取问题。
